...
def main(args):
    ...
    for i in ary:
        if i[0]<2:
            pth.append(image_files[i[1]])
    print(pth)

    for i in pth:
        shutil.copy(i,args.along_path)
...
def load_and_align_data(image_paths, image_size, margin, gpu_memory_fraction):
    ...
    for image in tmp_image_paths:
       ...
       # img_eq = img_eq.astype(np.uint8).reshape(img.shape)
        img_eq=img
        img_size = np.asarray(img_eq.shape)[0:2]
        bounding_boxes, _ = align.detect_face.detect_face(img_eq, minsize, pnet, rnet, onet, threshold, factor)
        ...
        cropped = img_eq[bb[1]:bb[3],bb[0]:bb[2],:]
...
def parse_arguments(argv):
    ...
    parser.add_argument('along_path', type=str, help='the_along path')
